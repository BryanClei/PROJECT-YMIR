...
class JobOrderController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        while (($data = fgetcsv($handle)) !== false) {
            ...
            $setting = JobOrder::firstOrCreate($settingData);
            ...
            $existing = JobOrderApprovers::where(
                "approver_name",
                $row["approver"]
            )
                ->where("approver_id", $user_id)
                ->where("layer", $row["layer"])
                ->where("job_order_id", $setting->id)
                ->first();
        ...
        return response()->json([
            "message" => "Jr Approver import completed.",
            "created" => $imported,
            "updated" => $updated,
            "skipped" => $skipped,
